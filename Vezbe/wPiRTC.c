...
//PIN na kome se nalazi senzor i dugme DVK512 plocici u wiringPi oznaci
char SENSOR = 29;
char BUTTON = 24;
...
{	
	...
	{	if(enable == 0)
		{
			sensor_value = digitalRead(SENSOR); //citamo vrednost sa senzora 1 je ako je detektovao nesto , 0 ako nije 
			if(sensor_value == 1)
			{
				enable = 1;
				detc_hours = hours;
			...
			}else
				{
					...
					lcdPrintf(lcd_h,"No Movment"); //ispisujemo na toj poziciji dati string
			...
			{	
				...
				lcdPosition(lcd_h, 12, 1);
				lcdPrintf(lcd_h,txt_seconds);
				printf("Movment detected at : %d %d %d \n", dec_hours, dec_minutes, dec_seconds);
